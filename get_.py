...
def get_var(files, var, snap):
    ...
    if snap in ['13', '12', '11', '10', '9', '8']:
        kill = np.array([])
        out = Parallel(n_jobs = 8)(delayed(get_)(i, ['Type', snap]) for i in filess)
...
def get_median(x, y, n = 28):
    
    
    bins = 10**(np.linspace(min(np.log10(x))-0.03, max(np.log10(x))+0.03, n))
    xx = np.zeros(len(bins)-1)
    yy = np.zeros(len(bins)-1)
    yy_up = np.zeros(len(bins)-1)
    yy_low = np.zeros(len(bins)-1)

    for i in range(0, len(bins)-1):
        ok = np.logical_and(x>=bins[i], x<bins[i+1])
        if np.sum(ok)>10:
            xx[i] = np.nanmedian(x[ok])
            yy[i] = np.nanmedian(y[ok])
            
            yy_low[i] = np.percentile(y[ok], 16)
            yy_up[i] = np.percentile(y[ok], 84)

    return xx, yy, yy_up, yy_low

def get_density(x, y):
    
    x_edges = 10**(np.arange(min(np.log10(x))-0.1, max(np.log10(x))+0.1, 0.1))
    y_edges = 10**(np.arange(min(np.log10(y))-0.1, max(np.log10(y))+0.1, 0.1))
