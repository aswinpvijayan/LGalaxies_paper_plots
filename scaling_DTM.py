...
def model(z, theta):
    ...
    x, y = z
    theta = unumpy.nominal_values(theta)
    a, b, c, d, e = theta
    tau = 5e-5/(a*x)
    return np.log10(a + (b-a)*(1.-np.exp(-c*(x**d)*((y/tau)**e))))

def model_cf(z, b, c, d, e):

    a = np.log10(8e-3)
    tau = 5e-5/((10**a)*z[0])
    return a + np.log10(1. + b*(1. - np.exp(-c*(z[0]**d)*((z[1]/tau)**e))))
...
p0 = [81.80954531, 0.08463305, -1.05679561, 1.97334825]
popt, pcov = curve_fit(model_cf, x_obs, y_obs, p0, sigma = np.ones(len(DTM))*0.00001)
print (popt)
...
sol = unumpy.uarray([D0, unumpy.nominal_values(D1), unumpy.nominal_values(alpha), unumpy.nominal_values(beta), unumpy.nominal_values(gamma)], [0.000001, unumpy.std_devs(D1), unumpy.std_devs(alpha), unumpy.std_devs(beta), unumpy.std_devs(gamma)])
...
def chisquare(x, y):
    ...
    return np.sum(((x-y)**2)/(len(x)-5-1))
    
print ("chi-square = {}".format(chisquare(np.log10(DTM), model_cf([Z, Age], *popt))))
...
ok = np.where(z < 3)
x, y = np.log10(DTM[ok]), model([Z[ok], Age[ok]], sol)
...
axs.set_ylim((-2.5, 0.1))
axs.set_xlim((-2.5, 0.1))
