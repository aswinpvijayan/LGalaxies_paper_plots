...
#Dust production rate as a function of redshift, only MR is plotted in this 

inp = int(sys.argv[1])  #0 - Cosmic dust rate versus redshift, 1 - Dust production rate versus Stellar Mass for different redshifts

if inp == 0:
    xlab = r'$z$'
    ylab = r'$log_{10}(\Phi_{DR}/(M_{\odot}yr^{-1}Mpc^{-3}))$'
    savename1 = 'dust_prod_rate_redshift.png'
elif inp == 1:
    xlab = r'$log_{10}(M_{*}/(M_{\odot}))$'
    ylab = r'$log_{10}(\Phi_{DR}/(M_{\odot}yr^{-1}))$'
    savename1 = 'dust_prod_rate_Mstar.png'
else:
    print ('Not an applicable plot choice, retry....')
    sys.exit()  

turn_on = int(sys.argv[3]) #If turn_on = 1, plot MRII if anything else then plot MR
if turn_on == 1:
    savename = 'MRII_'
else:
    savename = 'MR_'
...
if inp == 0:
    ...
    for i, z in enumerate(z_):
        ...
        axs[i].text(7.75, -5.5, r'$z = {}$'.format(z), fontsize = 18)
...
plt.savefig(savename+savename2+savename1)
