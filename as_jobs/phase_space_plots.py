...
turn_on = int(sys.argv[2]) # 1 in order to include MRII'))
...
for z in range(0, 9):
    ...
    if inp == 0:
        ...
        axs[z].text(6.75, 9, r'$z = {}$'.format(z), fontsize = 18)
    ...
    if inp == 2:
        ...
        axs[z].text(6.95, -1, r'$z = {}$'.format(z), fontsize = 18)
    ...
    if inp == 3:
        ...
        axs[z].text(4.9, -1, r'$z = {}$'.format(z), fontsize = 18)
        
        DG_met_obs(axs[z], z)   #Plotting the observational data points
      
    axs[z].scatter(x, y, marker = 'o', s=10, c = den, edgecolors='None', alpha = 0.1, cmap = plt.cm.get_cmap('gist_gray'))
    
    axs[z].plot(xx, yy, lw = 2, color = 'black')
    axs[z].plot(xx, yy_up, lw = 2, ls = 'dashed', color = 'black')
    axs[z].plot(xx, yy_low, lw = 2, ls = 'dashed', color = 'black')
...
plt.savefig(savename+namex+'.png')
